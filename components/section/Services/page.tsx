...
export default function Services({ data }: ServicesProps) {
  return (
    <section className="py-8 lg:py-10 bg-[#0a0b0e] text-white overflow-hidden">
        ...
        <div className="flex flex-col xl:flex-row justify-between items-center mb-10 gap-6 xl:gap-10">
          <div className="w-full xl:w-auto">
            ...
            <motion.h2
              ...
              viewport={{ once: true }}
              className="text-3xl md:text-[40px] xl:text-[42px] font-bold leading-[1.2] sm:leading-[1.1] tracking-tight"
            >
              Explore Our Full Range of <br className="hidden sm:block" />
              <span className="text-[#D49A4D]">Event Solutions</span>
            </motion.h2>
          ...
          >
            <p className="max-w-full sm:max-w-[340px] text-gray-300 leading-[1.7] text-[15px]">
              {data.description}
            </p>
            <Link
              href="#"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
          {data.items.map((service, index) => (
            <motion.div
              ...
              className="group flex flex-row bg-[#0c0d10] border border-[#D49A4D]/20 rounded-[24px] overflow-hidden hover:border-[#D49A4D]/50 hover:shadow-[0_0_20px_rgba(212,154,77,0.1)] transition-all h-[200px] lg:h-[240px]"
            >
              {/* LEFT SIDE (TEXT) */}
              <div className="flex-[0.55] p-5 sm:p-6 pr-2 flex flex-col justify-between">
                <div>
                  <h3 className="text-[18px] sm:text-[22px] font-medium text-white leading-[1.3] mb-3 sm:mb-4 pr-1 sm:pr-2">
                    {service.title}
                  </h3>
